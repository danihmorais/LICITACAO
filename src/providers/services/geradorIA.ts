...
const REGRAS_MINIMAS_TEXTO: Record<string, string> = {
    ...
    "PROVIDENCIAS_CONT": "Mínimo de 2 parágrafo. Fale sobre instrução de fiscais, planejamento de fiscalização, e demais providências relacionadas à fiscalização e controle da execução contratual, bem como à mitigação de riscos relacionadas à contratação e diálogo com a autoridade competente (Prefeito) para adoção de providências em caso de necessidade de revisão contratual, aplicação de sanções, ou outras medidas administrativas relacionadas à contratação.",
...
Não utilize markdown, placeholders ou listas numeradas salvo quando expressamente necessário.
Não explique o JSON e retorne exclusivamente um objeto JSON válido onde cada chave deve conter apenas o texto correspondente, sem campos vazios.
...
EXIGÊNCIA DE AMOSTRA: ${amostraFormatada}
EXIGÊNCIA DE VISTORIA: ${vistoriaFormatada}`;
...
A JUSTIFICATIVA deve conter motivação administrativa detalhada, demonstração do interesse público, necessidade institucional, impactos da não contratação, alinhamento ao planejamento e continuidade administrativa.`;
        ...
        if (modalidadeTipo === "PREGAO_ELETRONICO") {
            ...
            diretrizModalidade = `Justifique tecnicamente a contratação direta utilizando como base: ${motivoModalidadeRaw}`;
        } else if (modalidadeTipo === "PREGAO_PRESENCIAL") {
            diretrizModalidade = `Justifique a excepcionalidade do pregão presencial utilizando como base: ${motivoModalidadeRaw}`;
        }
